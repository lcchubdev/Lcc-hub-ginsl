...
Tab16:AddButton({
    ...
    Callback = function()
       loadstring(game:HttpGet("https://raw.githubusercontent.com/lcchubdev/Painel-admm/refs/heads/main/README.md"))()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Painel Admin",
...
local Tab1 = Window:MakeTab({"Scripts fe", "Cloud"})

Tab1:AddButton({"Skybox fe", function()
    loadstring(game:HttpGet("https://rawscripts.net/raw/Brookhaven-RP-Skybox-Brookhaven-61921"))()
end})

Tab1:AddButton({"Emote", function()
    loadstring(game:HttpGet("https://rawscripts.net/raw/Brookhaven-RP-Brookhaven-Fe-Vip-Emotes-45890"))()
end})

local Tab1 = Window:MakeTab({"Op", "sword"})

Tab1:AddButton({"Fazer ilha", function(Value)
print("Hello World!")
end})
--// 🌴🏰 Ilha Secreta Global no Céu (SEM TELEPORTE)
-- Criadores: Shelby & Hokagerei | Executor: Delta

local workspaceGlobal = game:GetService("Workspace")

local function criarParteGlobal(nome, pos, tam, cor, mat)
	...
	p.CanCollide = true
	p.Parent = workspaceGlobal
	return p
end

-- ===== ILHA PRINCIPAL =====
local posIlha = Vector3.new(0, 1250, 0)

local ilha = criarParteGlobal("🌴IlhaSecretaGlobal", posIlha, Vector3.new(350, 5, 350), Color3.fromRGB(50,200,50), Enum.Material.Grass)

-- Rochas da borda
for i = 1, 14 do
	local ang = math.rad(i * 25)
	local offset = Vector3.new(math.cos(ang)*160, -5, math.sin(ang)*160)
	local pedra = criarParteGlobal("Pedra"..i, posIlha+offset, Vector3.new(60,25,60), Color3.fromRGB(80,80,80), Enum.Material.Slate)
	pedra.CFrame = pedra.CFrame * CFrame.Angles(math.rad(math.random(-15, 15)),0,math.rad(math.random(-15,15)))
end
...
for i = 1, 12 do
	...
	local tronco = criarParteGlobal("Tronco"..i, posIlha+Vector3.new(x,10,z), Vector3.new(4,20,4), Color3.fromRGB(92,64,51), Enum.Material.Wood)
	local folhas = criarParteGlobal("Folhas"..i, tronco.Position+Vector3.new(0,15,0), Vector3.new(25,20,25), Color3.fromRGB(60,200,60), Enum.Material.Grass)
	folhas.Shape = Enum.PartType.Ball
end

-- Nuvens flutuando
for i = 1, 25 do
	local cloud = criarParteGlobal("Nuvem"..i, posIlha + Vector3.new(math.random(-220,220), math.random(20,70), math.random(-220,220)),
		Vector3.new(math.random(80,180), math.random(20,40), math.random(80,180)),
		Color3.fromRGB(255,255,255), Enum.Material.ForceField)
	cloud.Transparency = 0.4
end

-- ===== CASTELO =====
local posCastelo = posIlha + Vector3.new(0, 10, 0)
local alturaMuro = 80
local larguraCastelo = 250
local espessura = 12

-- Muralhas
criarParteGlobal("MuroFrente", posCastelo + Vector3.new(0,alturaMuro/2,-larguraCastelo/2), Vector3.new(larguraCastelo,alturaMuro,espessura), Color3.fromRGB(130,130,130), Enum.Material.Concrete)
criarParteGlobal("MuroTras", posCastelo + Vector3.new(0,alturaMuro/2,larguraCastelo/2), Vector3.new(larguraCastelo,alturaMuro,espessura), Color3.fromRGB(130,130,130), Enum.Material.Concrete)
criarParteGlobal("MuroDir", posCastelo + Vector3.new(larguraCastelo/2,alturaMuro/2,0), Vector3.new(espessura,alturaMuro,larguraCastelo), Color3.fromRGB(130,130,130), Enum.Material.Concrete)
criarParteGlobal("MuroEsq", posCastelo + Vector3.new(-larguraCastelo/2,alturaMuro/2,0), Vector3.new(espessura,alturaMuro,larguraCastelo), Color3.fromRGB(130,130,130), Enum.Material.Concrete)

-- Portão
criarParteGlobal("Portao", posCastelo+Vector3.new(0,25,-larguraCastelo/2+6), Vector3.new(40,50,2), Color3.fromRGB(90,60,40), Enum.Material.WoodPlanks)

-- Torres + bandeiras com vento
local cantos = {
	Vector3.new(larguraCastelo/2 - 18,0,larguraCastelo/2 - 18),
	Vector3.new(-larguraCastelo/2 + 18,0,larguraCastelo/2 - 18),
	Vector3.new(larguraCastelo/2 - 18,0,-larguraCastelo/2 + 18),
	Vector3.new(-larguraCastelo/2 + 18,0,-larguraCastelo/2 + 18)
}

for i, pos in ipairs(cantos) do
	local torre = criarParteGlobal("Torre"..i, posCastelo + pos + Vector3.new(0,40,0), Vector3.new(35,90,35), Color3.fromRGB(100,100,100), Enum.Material.Concrete)
	criarParteGlobal("TopoTorre"..i, torre.Position + Vector3.new(0,50,0), Vector3.new(45,12,45), Color3.fromRGB(70,70,70), Enum.Material.Metal)

	local bandeira = criarParteGlobal("Bandeira"..i, torre.Position + Vector3.new(0,50,10), Vector3.new(12,8,1), Color3.fromRGB(255,0,0), Enum.Material.Neon)
	coroutine.wrap(function()
		while true do
			bandeira.CFrame = bandeira.CFrame * CFrame.Angles(0, math.rad(math.random(-3,3)),0)
			wait(0.25)
		end
	end)()
end
...
-- Piso interno
criarParteGlobal("PisoCastelo", posCastelo + Vector3.new(0,1,0), Vector3.new(larguraCastelo-10,2,larguraCastelo-10), Color3.fromRGB(120,120,120), Enum.Material.Concrete)

-- Trono
local trono = criarParteGlobal("Trono", posCastelo + Vector3.new(0,15,60), Vector3.new(12,18,12), Color3.fromRGB(255,215,0), Enum.Material.Neon)
criarParteGlobal("TronoEncosto", trono.Position + Vector3.new(0,15,-4), Vector3.new(12,20,2), Color3.fromRGB(255,215,0), Enum.Material.Neon)

-- NPC parado no trono
criarParteGlobal("ReiNPC", trono.Position + Vector3.new(0,12,0), Vector3.new(4,10,2), Color3.fromRGB(200,0,0), Enum.Material.SmoothPlastic)

-- Baú com brilho
local bau = criarParteGlobal("BauSecreto", posCastelo + Vector3.new(25,3,25), Vector3.new(6,6,6), Color3.fromRGB(255,215,0), Enum.Material.Neon)
local luz = Instance.new("PointLight", bau)
...
-- Fogueiras
for i=1,6 do
	local fog = criarParteGlobal("Fogueira"..i, posCastelo+Vector3.new(math.random(-100,100),2,math.random(-100,100)), Vector3.new(4,4,4), Color3.fromRGB(255,140,0), Enum.Material.Neon)
	local fogo = Instance.new("ParticleEmitter", fog)
	fogo.Color = ColorSequence.new(Color3.fromRGB(255,140,0), Color3.fromRGB(255,60,0))
	fogo.Size = NumberSequence.new(2,0)
	fogo.Speed = NumberRange.new(2,5)
	fogo.Rate = 15
	fogo.Lifetime = NumberRange.new(1,2)
end

-- Música Vox Seas
local musica = Instance.new("Sound", workspaceGlobal)
musica.SoundId = "rbxassetid://6031094490"
musica.Volume = 1
musica.Looped = true
musica:Play()

print("🌤️ Ilha secreta global criada sem teleporte!")

Tab1:AddButton({"Fazer ilha V2", function()

    --== ILHA SECRETA COMPLETA / BROOKHAVEN / VISÍVEL PARA TODOS ==--
    ...
    ----------------------------------------------------------------
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Clouds") or obj.Name:lower():find("cloud") then
            obj:Destroy()
        end
    end
    ...
    -- CRIA A ILHA
    ----------------------------------------------------------------
    local ilha = Instance.new("Part")
    ilha.Name = "IlhaSecreta"
    ilha.Size = Vector3.new(800, 30, 800) 
    ilha.Position = Vector3.new(0, 500, 0)
    ilha.Anchored = true
    ilha.Material = Enum.Material.Sand
    ilha.Color = Color3.fromRGB(194, 178, 128)
    ilha.Parent = Workspace

    ----------------------------------------------------------------
    -- EFEITO DE VENTO NA GRAMA
    ----------------------------------------------------------------
    local grama = Instance.new("Part")
    grama.Size = Vector3.new(800, 1, 800)
    grama.Position = ilha.Position + Vector3.new(0, 0.6, 0)
    grama.Anchored = true
    grama.Material = Enum.Material.Grass
    grama.Color = Color3.fromRGB(101, 180, 90)
    grama.Parent = Workspace

    local vento = Instance.new("ParticleEmitter")
    vento.Texture = "rbxassetid://688058414" 
    vento.Rate = 5
    vento.Lifetime = NumberRange.new(3)
    vento.Speed = NumberRange.new(3)
    vento.SpreadAngle = Vector2.new(10,10)
    vento.Parent = grama

    ----------------------------------------------------------------
    -- CASTELO GRANDE
    ----------------------------------------------------------------
    local function CriarBloco(p, size, pos, color)
    ...
    local castelo = Instance.new("Folder", Workspace)
    castelo.Name = "CasteloSecreto"

    -- Base maior
    CriarBloco(castelo, Vector3.new(200, 20, 200), ilha.Position + Vector3.new(0, 20, 0), Color3.fromRGB(110, 110, 110))

    -- Torres
    for i, pos in ipairs({
        Vector3.new(80, 60, 80),
        Vector3.new(-80, 60, 80),
        Vector3.new(80, 60, -80),
        Vector3.new(-80, 60, -80)
    }) do
        CriarBloco(castelo, Vector3.new(40, 120, 40), ilha.Position + pos, Color3.fromRGB(120, 120, 120))
    end

    -- Torre central gigante
    CriarBloco(castelo, Vector3.new(90, 200, 90), ilha.Position + Vector3.new(0, 120, 0), Color3.fromRGB(105,105,105))

    ----------------------------------------------------------------
    -- NPC NO TRONCO PARADO
    ----------------------------------------------------------------
    local tronco = Instance.new("Part")
    tronco.Size = Vector3.new(6, 20, 6)
    tronco.Anchored = true
    tronco.Color = Color3.fromRGB(90, 60, 40)
    tronco.Material = Enum.Material.Wood
    tronco.Position = ilha.Position + Vector3.new(0, 20, 120)
    tronco.Parent = Workspace

    local npc = Instance.new("Part")
    npc.Size = Vector3.new(3, 6, 3)
    npc.Anchored = true
    npc.Color = Color3.fromRGB(255, 230, 150)
    npc.Position = tronco.Position + Vector3.new(0, 5, 0)
    ...
    brilho.Color = Color3.fromRGB(0, 255, 255)
    brilho.Brightness = 5
    brilho.Range = 40
    brilho.Parent = npc
    ...
    musica.SoundId = "rbxassetid://18425128818"
    musica.Looped = true
    musica.Volume = 3
    musica:Play()

    ----------------------------------------------------------------
    -- TODOS CONSEGUEM VER A ILHA
    ----------------------------------------------------------------
